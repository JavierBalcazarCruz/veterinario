-- ============================================================================
-- MIGRACIÓN DE BASE DE DATOS - SISTEMA DE CITAS COMPLETO
-- Fecha: 2025-10-30
-- Descripción: Actualiza tabla de citas médicas y crea tabla de estética
-- ============================================================================

-- 1. ACTUALIZAR TABLA DE CITAS MÉDICAS
-- Agregar estado 'en_curso' al enum existente
ALTER TABLE `citas`
...
-- Agregar nuevos campos para funcionalidad extendida
ALTER TABLE `citas`
ADD COLUMN `motivo_cancelacion` TEXT NULL AFTER `notas`,
ADD COLUMN `observaciones_finales` TEXT NULL AFTER `motivo_cancelacion`,
ADD COLUMN `confirmada_por_cliente` BOOLEAN DEFAULT FALSE AFTER `observaciones_finales`,
ADD COLUMN `fecha_confirmacion` TIMESTAMP NULL AFTER `confirmada_por_cliente`,
ADD COLUMN `recordatorio_enviado` BOOLEAN DEFAULT FALSE AFTER `fecha_confirmacion`,
ADD COLUMN `fecha_recordatorio` TIMESTAMP NULL AFTER `recordatorio_enviado`,
ADD COLUMN `duracion_minutos` INT DEFAULT 30 AFTER `fecha_recordatorio`;

-- 2. CREAR TABLA DE CITAS DE ESTÉTICA
...
-- 3. CREAR TABLA DE PERFILES DE ESTILO (Preferencias de cada mascota)
...
-- 4. CREAR TABLA DE GALERÍA DE FOTOS
...
-- 5. CREAR TABLA DE HORARIOS DE TRABAJO (Para disponibilidad)
...
-- 6. INSERTAR HORARIOS DE TRABAJO POR DEFECTO
-- (Lunes a Viernes: 8:00 - 18:00, Sábado: 9:00 - 14:00)
INSERT INTO `horarios_trabajo` (`id_doctor`, `dia_semana`, `hora_inicio`, `hora_fin`, `tipo`, `activo`) VALUES
(NULL, 'lunes', '08:00:00', '18:00:00', 'ambos', TRUE),
(NULL, 'martes', '08:00:00', '18:00:00', 'ambos', TRUE),
(NULL, 'miercoles', '08:00:00', '18:00:00', 'ambos', TRUE),
(NULL, 'jueves', '08:00:00', '18:00:00', 'ambos', TRUE),
(NULL, 'viernes', '08:00:00', '18:00:00', 'ambos', TRUE),
(NULL, 'sabado', '09:00:00', '14:00:00', 'ambos', TRUE);

-- 7. CREAR ÍNDICES ADICIONALES PARA OPTIMIZACIÓN
CREATE INDEX idx_citas_doctor_fecha ON `citas` (`id_doctor`, `fecha`);
...
SELECT COUNT(*) AS total_citas_medicas FROM `citas`;
SELECT COUNT(*) AS total_citas_estetica FROM `citas_estetica`;
SELECT COUNT(*) AS total_horarios_trabajo FROM `horarios_trabajo`;
